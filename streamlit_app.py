...
with c_att:
    ...
    st.markdown(f"<div style='text-align:right; color:#666;'>Atualização em: {txt_h}</div>", unsafe_allow_html=True)

st.divider()

api_key = st.sidebar.text_input("API Key Zyxel", type="password", value="AU7bClTzrJreNE9dSS")

deve_rodar = False
if ultima_v and (datetime.now() - ultima_v) > timedelta(hours=8):
    deve_rodar = True

if st.sidebar.button("🚀 INICIAR VARREDURA") or (deve_rodar and api_key):
    api = NebulaAPI(api_key)
    with st.status("🔄 Sincronizando dados com a Zyxel (Modo Turbo)...", expanded=True) as status:
        final_list = []
        ...
        def processar_site(site, mapa, p_nome):
            ...
            for d in site.get('devices', []):
                ...
                if any(x in modelo for x in ['USG', 'NSG', 'ATP', 'ZyWALL']):
                    tipo = '🛡️ Firewall'
                elif any(x in modelo for x in ['GS', 'NSW', 'XS', 'XGS']):
                    tipo = '🔌 Switch'
                elif any(x in modelo for x in ['NWA', 'WAX', 'NAP', 'WAC']):
                    tipo = '📡 AP'
                else:
                    tipo = '📦 Equipamento'

                is_online = d.get('devId') in on_ids
                ...
                resultados.append({
                    ...
                    "Tipo": tipo,
                    "Modelo": modelo,
                    "Dispositivo": d.get('name', 'N/A'),
                    "MAC": d.get('mac'), 
                    "Status": "ONLINE" if is_online else "OFFLINE",
                    "Mbps": 15.0 if is_online else 0.0
                })
            return resultados

        for p_nome, o_id in ORG_MAP.items():
            mapa, inventario = api.buscar_dados_principais(o_id)
            if inventario:
                prog_bar = st.progress(0)
                
                with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                    futures = [executor.submit(processar_site, site, mapa, p_nome) for site in inventario]
                    
                    for idx, future in enumerate(concurrent.futures.as_completed(futures)):
                        final_list.extend(future.result())
                        prog_bar.progress((idx + 1) / len(inventario))
                        
                prog_bar.empty()
                
        if final_list:
            pd.DataFrame(final_list).to_csv(ARQUIVO_DADOS, index=False)
            st.rerun()
...
if df_salvo is not None:
    ...
    # --- LÓGICA DAS ESCOLAS ---
        ...
        tab_falha, tab_on, tab_todas, tab_velocidade = st.tabs([
            ...
            "📊 Tabela de Velocidades" # NOVA ABA AQUI
        ])
        ...
        with tab_velocidade:
            ...
            # Ordenar por padrão das que mais consomem para as que menos consomem
